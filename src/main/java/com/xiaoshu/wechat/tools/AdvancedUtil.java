...
import com.xiaoshu.util.DateUtils;
...
import com.xiaoshu.wechat.pojo.WeixinUserList;
import com.xiaoshu.wechat.pojo.WeixinMedia;
import com.xiaoshu.wechat.pojo.WeixinGroup;
...
public class AdvancedUtil {
	...
	public static WeixinOauth2Token getOauth2AccessToken(String appId, String appSecret, String code) {
		...
		// 拼接请求地址
		String requestUrl = WeChatAPI.Oauth2AccessToken;
		...
		if (null != jsonObject) {
			try {
				...
				wat.setExpires_in(jsonObject.getInt("expires_in"));
	 ...
	 * @return SNSUserInfo
	 */
	public static FocusedUserInfo getSNSUserInfo2(String accessToken, String openId) {
		FocusedUserInfo snsUserInfo = null;
		// 拼接请求地址
		...
		if (null != jsonObject) {
			try {
				snsUserInfo = new FocusedUserInfo();
				// 用户的标识
	 ...
	 */
	public static FocusedUserInfo getUserInfo(String accessToken, String openId) {
		FocusedUserInfo user = null;
		if(accessToken != null && !"".equals(accessToken) && openId != null && !"".equals(openId)){//参数检查
			// 拼接请求地址
			String requestUrl = WeChatAPI.USER_INFO;
			requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken).replace("OPENID", openId);
			// 获取用户信息
			JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "GET", null);
			if (null != jsonObject) {
				try {
					user = new FocusedUserInfo();
					// 用户的标识
...
//					log.error("获取用户信息失败 errcode:{} errmsg:{}", errorCode, errorMsg);
	 ...
	 * @return SNSUserInfo
	 */
	public static FocusedUserInfo getSNSUserInfo(String accessToken, String openId) {
		FocusedUserInfo snsUserInfo = null;
		// 拼接请求地址
		String requestUrl = WeChatAPI.Oauth2UserInfo;
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken).replace("OPENID", openId);
		// 通过网页授权获取用户信息
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "GET", null);
	 ...
	 * @return Unionid
	 */
	public static String getUnionidUserInfo(String accessToken, String openId) {
		String requestUrl = WeChatAPI.UnionidUserInfo;
		...
		String json = ToolsHttpRequest.sendGet(requestUrl, param);
	...
	public static WeixinUserList getUserList(String accessToken, String nextOpenId) {
		...
		if (null != jsonObject) {
			try {
				...
				weixinUserList.setData(JSONArray.toList(dataObject.getJSONArray("openid"), List.class));
...
//		 * 获取用户信息
//		 */
//		User user = getUserInfo(accessToken, "oEdzejiHCDqafJbz4WNJtWTMbDcE");
