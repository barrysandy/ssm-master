...
public class ToolsString {
	...
	public static String getNonceStr() {
		...
		if(currentTime != null) {
			nonceStr = String.valueOf(currentTime.getTime()) + generateRandNumber(4);
			return nonceStr;
		}
		return nonceStr;
	}
...
//	}
	 ...
	 * 获取图片保存地址
	 * 
	 * @param appRoot 项目下的webRoot路径 获取方式：req.getSession().getServletContext().getRealPath("");
	 * @param webRoot 图片存放的初始不完整路径 获取方式："upload/CodeImg/"
	 ...
	 * 比较两个字符串并返回相同部分
	 * 
	 * @param str1 eg:"刘烨,孙坚,王二小,蜘蛛侠,钢铁侠,毛剑卿";
	 * @param str2 eg:"王二小,李占军,刘胡兰,毛剑卿";
	 ...
	 * @return
	 */
	public static String putOffBar(String str) {
		if(str != null){
			if(!"".equals(str)){
	...
	private static final String USER_CENTER_ICON = "user_center_icon";
	private static final String USER_CENTER_ICON_CSS = "<img src=\"http://www.daxi51.com/ssm/resources/img/icon/userCenter/usercenter.png\" style=\"height: 1.6rem;width: 1.6rem;vertical-align: middle;\"/>";


	/** 替换 USER_CENTER_ICON 为 USER_CENTER_ICON_CSS */
	public static String conversionIconCSSCode(String str) {
		if(str != null){
			if(!"".equals(str)){
				str = str.replaceAll(USER_CENTER_ICON,USER_CENTER_ICON_CSS);
			}
		}
		return str;
	}

	/** 替换 USER_CENTER_ICON_CSS 为 USER_CENTER_ICON */
	public static String conversionCSSCodeIcon(String str) {
		if(str != null){
			if(!"".equals(str)){
				str = str.replaceAll(USER_CENTER_ICON_CSS,USER_CENTER_ICON);
			}
		}
		return str;
	}
