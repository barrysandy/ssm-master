...
<head>
    ...
    <div id="app" class="rich_media">
        <div data-v-4fce1214="">
            <div data-v-4fce1214="" class="content" style="overflow-x: hidden;margin-top: -2.5rem;">
                <div data-v-4fce1214="" class="content-cell">
                    <h6 data-v-4fce1214="" style="font-size: 1.3rem;">选择组团日期</h6>
                    <div data-v-4fce1214="" style="font-size: 1.3rem;margin-top: 1.5rem;margin-bottom: 1.5rem;">
                        <c:forEach var="list" items="${groupPriceList}" varStatus="status">
                            <c:if test="${status.index == 0}">
                                <button style="font-size: 1.3rem;" id="time${status.index + 1}" type="button" class="btn btn-primary btn-sm" onclick="changePrice('${list.descM}',event,'${list.price}')">${list.descM}</button>
                            </c:if>
                            <c:if test="${status.index > 0}">
                                <button style="font-size: 1.3rem;" id="time${status.index + 1}" type="button" class="btn btn-light btn-sm" onclick="changePrice('${list.descM}',event,'${list.price}')">${list.descM}</button>
                            </c:if>
                        </c:forEach>
                    </div>
                </div>
        ...
        <input type="hidden" id="userUserTime" />
    ...
    <script type="text/javascript">
        ...
        /** 选择时间 */
        function changePrice(time,event,priceTemp) {
            $("#userUserTime").val(time);
            ...
            setTimeout(function () {
                ...
                $.post("${path}/shop/interfaceCanCreateGroupOrder",{id:id},function(result){
                    if(result == "ok"){
                         ...
                         */
                        var total = $("#Total").val();
                        //type = 'create' 表示下单类型为创建组团
                        $.post("${path}/order/placeAnOrderGroupInUser",{id:id,total:total,userPhone:userPhone,userUserTime:userUserTime,userName:userName,type:'create'},function(info){
