    ...
    <script>
        function add(menuId,id){
            layer.open({
                ...
                area: ['50%', '90%'],
                content: '${path}/activity/toAddScan?menuId='+ menuId + '&id='+ id
            });
        }

        function confirmDelete(userId,id) {
            layer.open({
                type: 1,
                title: '删除',
                skin: 'layui-layer-rim',
        ...
        function deleted(userId,id) {
            var url = "${path}/activity/delScan";
            $.post(url,{'userId':userId,'id':id},function(result){
                if(result.state) {
                    layer.open({
                        type: 1,
                        title: '移除',
                                        ...
                                        <tr>
                                            ...
                                            <td align="center">
                                                <button class="u-btn sm texture f-m-l-xs" title="移除" type="button"
                                                        onClick="confirmDelete('${bean.id}','${id}')">
