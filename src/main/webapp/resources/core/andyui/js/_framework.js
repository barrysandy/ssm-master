...
(function (andy) {
        ...
        //头部滑动位置的定位 chujianyang,取消了maskLeft, maskRight
        ...
        var topMenuStr = '<ul class="u-topmenuul">'
            ...
            + '<a href="{{if list[i].url}}{{list[i].path}}{{else}}javascript:void(0){{/if}}" target={{list[i].target||centerIframeID}} index="{{i}}" class="rootmenu">'
            ...
            + '<a  href="{{if list[i].children[j].path}}{{list[i].children[j].path}}{{else}}javascript:void(0){{/if}}" index="{{i}}_{{j}}" target={{list[i].children[j].target||centerIframeID}} class="">{{list[i].children[j].name}}{{if list[i].children[j].children}}<i class="iconfont moreicon"></i>{{/if}}</a>'
            + '<ul style="display: none;">'
            + '{{each list[i].children[j].children as value k}}'
            + '<li id="{{list[i].children[j].children[k].id}}" class="mor"><a href={{list[i].children[j].children[k].path}} index="{{i}}_{{j}}_{{k}}" target={{list[i].children[j].children[k].target||centerIframeID}} class="">{{list[i].children[j].children[k].name}}</a></li>'
            + '{{/each}}'
            + '</ul>'
            + '</li>'
            + '{{/each}}'
        ...
        var leftMenuStr = ' {{each list as value i}}'
            ...
            + '<a href="{{if list[i].children[j].path}}{{list[i].children[j].path}}{{else}}javascript:void(0){{/if}}" index="{{i}}_{{j}}" target="{{list[i].children[j].target||centerIframeID}}">{{if list[i].children[j].icons}}<i class="iconfont classicon" title="{{list[i].children[j].name}}">{{list[i].children[j].icons}}</i>{{/if}}'
            ...
            + '<a href = "{{if list[i].children[j].children[k].path}}{{list[i].children[j].children[k].path}}{{else}}javascript:void(0){{/if}}" index="{{i}}_{{j}}_{{k}}" target="{{list[i].children[j].children[k].target||centerIframeID}}">'
            + '<i class="iconfont classicon" title="{{list[i].children[j].children[k].name}}">{{list[i].children[j].children[k].icons}}</i>{{list[i].children[j].children[k].name}}{{if list[i].children[j].children[k].children}}<i class="iconfont moreicon">&#xe607;</i>{{/if}}</a>'
            ...
            + '<a href = "{{list[i].children[j].children[k].children[l].path}}" index="{{i}}_{{j}}_{{k}}_{{l}}" target="{{list[i].children[j].target||centerIframeID}}">{{list[i].children[j].children[k].children[l].name}}{{if list[i].children[j].children[k].children[l].children}}<i class="iconfont moreicon">&#xe607;</i>{{/if}}</a>'
            + '</li>'
            + '{{/each}}'
            + '</ul>'
            + '{{/if}}'
            ...
            + '{{/if}}'
            + '{{/each}}';
        var pathCfg = '{{each list as value i}} {{if list[i].name}} <li {{if i===list.length-1}}class="activate"{{/if}}> <a href="javascript:void(0)" target="stage">{{list[i].name}}</a> {{if i!==list.length-1}}<i class="iconfont"></i>{{/if}}</li>{{/if}}{{/each}}';
        /**
...
//
        ...
        module.refreshPath = function (e, parent, mainUl) {
            ...
            for (i; i < indexs.length; i++) {
                ...
                if (data) {
                    ...
                    if (data.pathPre) {
                        list.push({name: data.pathPre ,path: ""});
                    }
                    list.push(data);
                }
            }
            ...
            if (str.indexOf("首页") < 0) {
                $(parent).prepend('<li class="activate home"><a href="' + mainUl + '" target="stage"><i class="iconfont homeicon">&#xe609;</i>首页<i class="iconfont"></i></a></li>');
            } else {
        ...
        module.refreshUnusualMenu = function (parent, index) {
            var list = [], getMenu;
            var str = "", sample = '{{each list as value i}} <li class="mor"> <a href="{{list[i].path}}" target="{{list[i].target}}" index="{{list[i].index}}">{{list[i].text}}</a></li>{{/each}}';
        ...
        module.leftMenu = function (parent, centerIframeID, chooseCallback, onlyLeftMenu) {
            ...
            //////如果三级菜单有子节点//////////////////////////////////////////////////////
            //鼠标移入
            $(document).on('mouseenter', '.u-sidemenu a', function (e) {
                var ull = $(this).next();
                ////如果不是上一级的子菜单,收起上一个菜单
