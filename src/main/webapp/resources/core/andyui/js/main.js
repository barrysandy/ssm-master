(function ($) {
    ...
    $.fn.an_main = function (option) {
        ...
        getFistForNav("#" + _stringid.topnavid, function (data) {
            ...
            if (_options.isBaseTopMenu) {
                ...
                appendIn(".u-topmenu", "<ul class='"+_stringclass.topnav+"'></ul>");
                createTopMenu();
            }
            ...
            if ($("#" + _stringid.topnavid + " >li").first().children("a").attr("href") == "#" && _options.isCloseIndex) {
                $("#" + _stringid.topnavid + " >li").first().children("a").attr({"href": _options.indexUrl, "target": _options.iframeName})
            }
            ...
            $(document).on('click', function (e) {
                ...
                if (element.parent().parent().attr("class") == _stringclass.topnav) {
    ...
    var createBreadcrumb = function () {
        if (_options.isBreadcrumb) {
            if($("body").find(_stringclass.path).length < 1){
    ...
    var menuTree = function (){
        ...
        //鼠标离开顶部菜单执行 除去一级菜单
        $(document).on('mouseleave', '#'+_stringid.topnavid, function (e) {
            var $ul = $("." + _stringclass.topnav).find('ul:visible');
            $ul.each(function (index, e) {
                $(e).animate({width: 'hide'}, "fast");
            })
        });
        ...
        // 顶部菜单二三级点击
        $(document).on('click', '#' + _stringid.topnavid + ' a', function(e){
            var $this = $(e.target);
            $this.is('a') || ($this = $this.closest('a'));//如果没有a 就返回a的第一个父级
            //清除老路径
            // $("."+_stringclass.path).empty();
        ...
        $(document).on('click', '#' + _stringid.menunavid + ' a', function (e) {
            ...
            // 强制修改iframe地址
            var iframe = $("#" + _options.iframeName);
    ...
    var createTopMenu = function () {
        for (var i = 0; i < _menujson.length; i++) {
            ...
            if (_options.menuInTop) {
                ...
                str = "<li class = '"+liclass+"' id = '" + data.id + "'><a href = 'javascript:void(0)' class = '"+activeclass+"'>" + beforicon + data.text + arrow + "</a></li>";
            } else {
                //创建联系侧边菜单顶部导航
                var liclass = '';
                var aclass = '';
            ...
            $("." + _stringclass.topnav).append(str);
    ...
    var showTopMenuOver = function () {
        ...
        //应该显示的宽度
        absolutWidth = bg.offsetWidth - padLeft - 40;
